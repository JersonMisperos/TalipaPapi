            ...
            <div class="card ">
              ...
              <div class="card-body">
                <div class="table-responsive ps">
                  <table class="table table-hover tablesorter " id="">
                    <thead class=" text-primary">
                    <tr><th>ID</th><th>Products</th><th>Available Stocks</th>
                    </tr></thead>
                    <tbody>
                      <?php 
                        $result=mysqli_query($con,"select * from products")or die ("query 1 incorrect.....");
                        $i=1;
                        while(list($cat_id,$cat_title,$product_desc,$product_title, $product_brand, $product_keywords )=mysqli_fetch_array($result))
                        {	
                            $sql = "SELECT COUNT(*) AS count_items FROM products WHERE product_desc=$i";
                            $query = mysqli_query($con,$sql);
                            $row = mysqli_fetch_array($query);
                            $count=$row["count_items"];
                            $i++;
                        echo "<tr><td>$cat_id</td><td>$product_title</td><td>$product_keywords</td>
